...
class RFPLoader:
    ...
    def __init__(self, user_id: str = "global"):
        ...
        self.doc_store = DocumentStore(user_id=user_id, registry=registry)
        self.vec_manager = VectorStoreManager(user_id=user_id, registry=registry)
    ...
    def process_file(self, file_obj, filename: str, chunk_size: int = None, chunk_overlap: int = None, table_strategy: str = None) -> str:
        ...
        # 1. 파일 임시 저장 & 해시 계산
        temp_dir = self.doc_store.raw_path
        ...
        print(f"Processed {filename} -> {doc_hash} (Chunks: {len(chunks)}, User: {self.user_id})")
        return doc_hash
