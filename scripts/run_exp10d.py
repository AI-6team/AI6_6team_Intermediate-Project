"""
EXP10d: Phase D — Hybrid Parser Experiment
V4_hybrid: hwp5txt 텍스트 + hwp5html 테이블 결합

Phase CE에서 V0(텍스트 강점)과 V1(테이블 강점)의 장점 결합.
V0 baseline과 비교하여 Baseline 고정 여부 판단.

실행: cd bidflow && python -u scripts/run_exp10d.py
"""
...
# ── Constants ──
EXP_DIR = PROJECT_ROOT / 'data' / 'exp10d'
...
def main():
    print(f"\n{'='*70}")
    print(f"EXP10d: Phase D — Hybrid Parser (V4) vs V0 Baseline")
    ...
    chunker = TableAwareChunker(
        ...
        table_mode="flat",  # flat for hybrid (simpler, V1 was better than V2/V3)
    )

    config_labels = ["V0_hwp5txt", "V4_hybrid"]
    index_stats = {}
    ...
    print(f"\n{'='*70}")
    print("V4_hybrid vs V0_hwp5txt Comparison")
    print(f"{'='*70}")
    for doc_key in DOC_CONFIGS:
        v0_kw = results_df[(results_df['config'] == 'V0_hwp5txt') & (results_df['doc_key'] == doc_key)]['kw_v2'].mean()
        v4_kw = results_df[(results_df['config'] == 'V4_hybrid') & (results_df['doc_key'] == doc_key)]['kw_v2'].mean()
        delta = v4_kw - v0_kw if not (np.isnan(v0_kw) or np.isnan(v4_kw)) else 0
        marker = "↑" if delta > 0.01 else ("↓" if delta < -0.01 else "→")
        print(f"  {doc_key} ({DOC_CONFIGS[doc_key]['doc_type']}): V0={v0_kw:.4f} → V4={v4_kw:.4f} ({marker}{delta:+.4f})")

    v0_overall = results_df[results_df['config'] == 'V0_hwp5txt']['kw_v2'].mean()
    v4_overall = results_df[results_df['config'] == 'V4_hybrid']['kw_v2'].mean()
    print(f"\n  OVERALL: V0={v0_overall:.4f} → V4={v4_overall:.4f} ({v4_overall - v0_overall:+.4f})")

    # ── Save ──
    results_df.to_csv('data/experiments/exp10d_metrics.csv', index=False, encoding='utf-8-sig')
    exp_report = {
        'experiment': 'exp10d_hybrid_parser',
        'phase': 'D',
        ...
        'results': {
            'v0_overall': round(v0_overall, 4),
            'v4_overall': round(v4_overall, 4),
            'delta': round(v4_overall - v0_overall, 4),
        },
    ...
    with open('data/experiments/exp10d_report.json', 'w', encoding='utf-8') as f:
        json.dump(exp_report, f, ensure_ascii=False, indent=2, default=str)

    print(f"\nSaved: data/experiments/exp10d_metrics.csv")
    print(f"Saved: data/experiments/exp10d_report.json")
    print(f"\n{'='*70}")
