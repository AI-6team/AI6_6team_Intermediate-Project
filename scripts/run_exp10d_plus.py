"""
EXP10d+: Phase D+ — Table Quality Filter Experiment
V5_hybrid_smart: V4 + 레이아웃 테이블 필터링

V4에서 doc_A가 -19.4pp 하락한 원인(레이아웃 테이블 노이즈)을 해결.
V4 vs V5 비교하여 doc_A 복구 + 테이블 문서 유지 검증.

실행: cd bidflow && python -u scripts/run_exp10d_plus.py
"""
...
# ── Constants ──
EXP_DIR = PROJECT_ROOT / 'data' / 'exp10d_plus'
...
def main():
    print(f"\n{'='*70}")
    print(f"EXP10d+: Phase D+ — Table Quality Filter (V5) vs V4 Hybrid")
    ...
    config_labels = ["V4_hybrid", "V5_hybrid_smart"]
    index_stats = {}
    ...
    for config_label in config_labels:
        ...
        for doc_key, doc_info in DOC_CONFIGS.items():
            ...
            if config_label == "V4_hybrid":
                docs = chunker.chunk_v4_hybrid(file_path)
            elif config_label == "V5_hybrid_smart":
                docs = chunker.chunk_v5_hybrid_smart(file_path)
            else:
    ...
    print(f"\n{'='*70}")
    print("TABLE FILTER EFFECT:")
    for doc_key in DOC_CONFIGS:
        v4_key = f"V4_hybrid_{doc_key}"
        v5_key = f"V5_hybrid_smart_{doc_key}"
        if v4_key in index_stats and v5_key in index_stats:
            v4_table = index_stats[v4_key]['n_table']
            v5_table = index_stats[v5_key]['n_table']
            removed = v4_table - v5_table
            print(f"  {doc_key}: V4 table={v4_table} → V5 table={v5_table} (removed {removed})")
    print(f"{'='*70}")
    ...
    # Compare V4 vs V5 per doc
    print(f"\n{'='*70}")
    print("V5_hybrid_smart vs V4_hybrid Comparison (per document)")
    print(f"{'='*70}")

    # Reference V0 from previous experiment
    v0_ref = {'doc_A': 0.9244, 'doc_B': 0.5933, 'doc_C': 0.6639, 'doc_D': 0.7153, 'doc_E': 0.8694}

    for doc_key in DOC_CONFIGS:
        v4_kw = results_df[(results_df['config'] == 'V4_hybrid') & (results_df['doc_key'] == doc_key)]['kw_v2'].mean()
        v5_kw = results_df[(results_df['config'] == 'V5_hybrid_smart') & (results_df['doc_key'] == doc_key)]['kw_v2'].mean()
        v0_kw = v0_ref.get(doc_key, float('nan'))

        delta_v5_v4 = v5_kw - v4_kw if not np.isnan(v5_kw) else 0
        delta_v5_v0 = v5_kw - v0_kw if not np.isnan(v5_kw) else 0

        marker = "↑" if delta_v5_v4 > 0.01 else ("↓" if delta_v5_v4 < -0.01 else "→")
        print(f"  {doc_key} ({DOC_CONFIGS[doc_key]['doc_type']}):")
        print(f"    V0={v0_kw:.4f} → V4={v4_kw:.4f} → V5={v5_kw:.4f} ({marker}{delta_v5_v4:+.4f} vs V4, {delta_v5_v0:+.4f} vs V0)")

    v4_overall = results_df[results_df['config'] == 'V4_hybrid']['kw_v2'].mean()
    v5_overall = results_df[results_df['config'] == 'V5_hybrid_smart']['kw_v2'].mean()
    v0_overall = np.mean(list(v0_ref.values()))
    print(f"\n  OVERALL: V0={v0_overall:.4f} → V4={v4_overall:.4f} → V5={v5_overall:.4f}")
    print(f"    V5 vs V4: {v5_overall - v4_overall:+.4f}")
    print(f"    V5 vs V0: {v5_overall - v0_overall:+.4f}")
    ...
    # ── Save ──
    results_df.to_csv('data/experiments/exp10d_plus_metrics.csv', index=False, encoding='utf-8-sig')
    exp_report = {
        'experiment': 'exp10d_plus_table_filter',
        'phase': 'D+',
        ...
        'results': {
            'v0_overall_ref': round(v0_overall, 4),
            'v4_overall': round(v4_overall, 4),
            'v5_overall': round(v5_overall, 4),
            'delta_v5_v4': round(v5_overall - v4_overall, 4),
            'delta_v5_v0': round(v5_overall - v0_overall, 4),
        },
    ...
    with open('data/experiments/exp10d_plus_report.json', 'w', encoding='utf-8') as f:
        json.dump(exp_report, f, ensure_ascii=False, indent=2, default=str)

    print(f"\nSaved: data/experiments/exp10d_plus_metrics.csv")
    print(f"Saved: data/experiments/exp10d_plus_report.json")
    print(f"\n{'='*70}")
    print(f"EXP10d+ COMPLETE")
    print(f"{'='*70}")
